...
async fn index(req: HttpRequest) -> impl Responder {
    ...
    log::info!("{} {:?} visiting callsoup - {:?}", txid, peer, requ);
...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("callsoup initialized at {} >>> callsoup HTTPS server on port 3333 using rustls TLSv1.3 and TLSv1.2", readi);
    HttpServer::new(|| {
        App::new()
    ...
    .bind_rustls("0.0.0.0:3333", config)?
    .run()
    .await
}
...
fn load_rustls_config() -> rustls::ServerConfig {
    ...
    if keys.is_empty() {
        ...
        eprintln!("{} - callsoup FATAL - Open of key.pem paired with cert.pem failed, server must shutdown. Use PKCS8 PEM compatible with rustls.", readu);
        std::process::exit(1);
    }
